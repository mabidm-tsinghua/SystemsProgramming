...
/* OpReg.exe -a HKEY_CUURET_USER\dcis\systemprogramming 
   OpReg.exe -d HKEY_CUURET_USER\dcis\systemprogramming
   OpReg.exe -a -v HKEY_CUURET_USER\dcis\systemprogramming
		...
		*/
		...
		if(valFlag){
			...
			_getts_s(valData,MAX_PATH);
				...
				/*RegSetKeyValue() is more convenient*/
			//RegSetValueEx() can also modify exisiting values
			
				if (RegSetValueEx (hSubKey,valName,0, REG_SZ,(BYTE*)valData, TSIZE * (_tcslen(valData)+1)) != ERROR_SUCCESS) //
		...
		if (RegDeleteKey(hKey,pScan)!= ERROR_SUCCESS) //
